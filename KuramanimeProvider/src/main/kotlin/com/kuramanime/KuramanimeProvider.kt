...
class KuramanimeProvider : MainAPI() {
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        for (i in 1..30) {
            ...
            val eps = Jsoup.parse(doc.select("#episodeLists").attr("data-content"))
                ...
                .mapNotNull {
                    ...
                    val episode = Regex("(\\d+[.,]?\\d*)")
                        .find(name)?.groupValues?.getOrNull(0)?.toIntOrNull()
        ...
        val recommendations = document.select("div#randomList > a").mapNotNull {
            ...
            val epPoster = it.select(".product__sidebar__view__item.set-bg")
                .attr("data-setbg")
    ...
    override suspend fun loadLinks(
        ...
        res.select("select#changeServer option").amap { source ->
            ...
            if (server.contains("kuramadrive", true)) {
                ...
                postRes.select("div.iframe-container iframe")
                    .attr("src")
                    .let { videoUrl ->
